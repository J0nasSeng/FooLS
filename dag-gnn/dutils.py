...
def load_data_(args, num_samples=10000, batch_size=1000):
    ...
    X = X.reshape((num_samples, args.data_variable_size, 1))
...
def create_3_nodes_fork(noise, f):
    X1 = noise()
    X0 = f(X1) + noise()
    X2 = f(X1) + noise()
...
def create_3_nodes_collider(noise, f):
    X0 = noise()
    X2 = noise()
    X1 = f(X0 + X2) + noise()
    gdata = np.array([X0, X1, X2]).T
...
def load_n_node_data(args, batch_size=128):
    #X = pd.read_csv('../data/n-var-case/linear/data/data_10_chain.csv', index_col=0).to_numpy()
    #X = create_n_node_chain(3, lambda: np.random.normal(0, 1, 10000), lambda x: 1.2*x)
    #X = create_10_nodes_fork(5, lambda: np.random.normal(0, 1, SAMPLE_SIZE), lambda x: 1.2*x)
    #X = create_3_nodes_fork(lambda: np.random.normal(0, 1, SAMPLE_SIZE), lambda x: 1.2*x)
    X = create_3_nodes_collider(lambda: np.random.normal(0, 1, SAMPLE_SIZE), lambda x: 1.2*x)
    #X = create_10_nodes_collider(5, lambda: np.random.normal(0, 1, SAMPLE_SIZE), lambda x: 1.2*x)

    scale_dict = {0: 3}
    
    if args.attack:
        X = rescale(X, scale_dict, True)
